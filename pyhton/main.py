...
def move_mouse(axis, value):
    """Move o mouse de acordo com o eixo e valor recebidos ou executa clique."""
    if axis == 0:
...
def controle(ser):
    """
    Lê dados da UART no formato: roll,pitch,yaw,click
    Move o mouse proporcional à variação de roll/pitch
    """
    last_roll = 0.0
    last_pitch = 0.0
    first_reading = True
    alpha = 0.3  # suavização dos deltas
    ...
    while True:
        try:
            ...
            if line.count(',') != 3:
                continue  # pula se não tiver 4 valores

            parts = line.split(",")
            try:
                roll = float(parts[0])
                pitch = float(parts[1])
                # yaw = float(parts[2])  # não usado
                click = int(parts[3])
            except ValueError:
                continue
            ...
            # Calcular delta (diferença de orientação)
            ...
            smoothed_dx = alpha * delta_roll + (1 - alpha) * smoothed_dx
            smoothed_dy = alpha * delta_pitch + (1 - alpha) * smoothed_dy

            # Ajuste de sensibilidade
            dx = int(smoothed_dx * 6)
            dy = int(-smoothed_dy * 6)  # invertido para mover de forma mais natural
